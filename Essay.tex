...
\section{Дополнительная теория} 
    ...
    Натуральным базисом, определяемым G K-векторного пространства $R/I$, назовем базис $B(G)$, элементами которого являются редуцированные мономы
...
\section{Алгоритм FGLM}
    ...
    \textbf{Определение:}  Обозначим $T(G) = (t_{ijk})$ матрицу $n \times D(I) \times D(I)$, чьи элементы:
    ...
    $m=x_lm'$, где $m'\in M(G)$ и $m'\langle m$. В этом случае, координаты $m'=x_sb_h$ по отношению к B(G), уже вычислены, и хранятся в $t_{i*k}$. 
    ...
    \textbf{Доказательство корректности:}
        ...
        Данный алгоритм представляет собой рассмотрение случаев, рассмотренных в доказательстве данной теоремы.

    \newpage
    \subsection{Смена упорядочения}
    \textbf{Теорема(2.1):} Пусть $I$ - нульмерный идеал и $(G_1, m_1)$ - его редуцированный 
    ...
    \textbf{Доказательство корректности:}
        ...
        тогда существует линейная комбинация $P = \sum \lambda_i*m_i$, такая, что $P\in \langle oldBasis\rangle$. Тогда
